...
class ModeloCategoriasModelos
{
	...
	static public function mdlConteos($idMarca = 0, $q = "")
	{
		...
		return array(
			...
			"clasificados" => max(0, $activos - $pendientes)
		);
	}
	...
	static public function mdlListar($filtros)
	{
		...
		$sql = "SELECT TRIM(m.modelo) AS modelo,
			...
			LEFT JOIN categoria_modelojf c ON c.id = s.id_categoria
	...
	static public function mdlAsignacionVigente($modelo, $pdo = null, $forUpdate = false)
	{
		...
		$sql = "SELECT ms.modelo, ms.id_subcategoria, ms.fecha_asignacion, ms.usuario_asignacion,
	...
	static public function mdlAsignar($modelo, $idSubcategoria, $usuarioId, $observacion = null, $origen = "pantalla")
	{
		...
		try {
			...
			if ($actual && (int) $actual["id_subcategoria"] === $idSubcategoria) {
				$pdo->commit();
				return array(
					"ok" => true,
					"idempotente" => true,
			...
			if ($actual) {
				...
				$upd->execute();
				$accion = "CAMBIO";
				$idAnterior = (int) $actual["id_subcategoria"];
			} else {
				$ins = $pdo->prepare(
					"INSERT INTO modelo_subcategoriajf
						(modelo, id_subcategoria, fecha_asignacion, usuario_asignacion)
					 VALUES (:modelo, :id_subcategoria, :ahora, :usuario)"
				);
			...
			$hist = $pdo->prepare(
				"INSERT INTO modelo_subcategoria_historialjf
					(modelo, id_subcategoria_anterior, id_subcategoria_nueva, accion, fecha, usuario_id, origen, observacion)
				 VALUES (:modelo, :id_ant, :id_nueva, :accion, :fecha, :usuario, :origen, :obs)"
			);
			$hist->bindValue(":modelo", $modelo, PDO::PARAM_STR);
			if ($idAnterior === null) {
				$hist->bindValue(":id_ant", null, PDO::PARAM_NULL);
			} else {
				$hist->bindValue(":id_ant", $idAnterior, PDO::PARAM_INT);
			}
			$hist->bindValue(":id_nueva", $idSubcategoria, PDO::PARAM_INT);
			$hist->bindValue(":accion", $accion, PDO::PARAM_STR);
			$hist->bindValue(":fecha", $ahora, PDO::PARAM_STR);
	...
	static public function mdlQuitar($modelo, $usuarioId, $origen = "pantalla")
	{
		...
		try {
			...
			$hist = $pdo->prepare(
				"INSERT INTO modelo_subcategoria_historialjf
					(modelo, id_subcategoria_anterior, id_subcategoria_nueva, accion, fecha, usuario_id, origen, observacion)
				 VALUES (:modelo, :id_ant, NULL, 'BAJA', :fecha, :usuario, :origen, NULL)"
			);
			$hist->bindValue(":modelo", $modelo, PDO::PARAM_STR);
			$hist->bindValue(":id_ant", (int) $actual["id_subcategoria"], PDO::PARAM_INT);
			$hist->bindValue(":fecha", $ahora, PDO::PARAM_STR);
			$hist->bindValue(":usuario", $usuarioId, $usuarioId === null ? PDO::PARAM_NULL : PDO::PARAM_INT);
			$hist->bindValue(":origen", substr((string) $origen, 0, 30), PDO::PARAM_STR);
			$hist->execute();
	...
	static private function mdlFormatearAsignacion($fila, $usuarioFallback = null)
	{
		...
		return array(
			"modelo" => $fila["modelo"],
			"id_subcategoria" => (int) $fila["id_subcategoria"],
	...
	static public function mdlHistorial($modelo)
	{
		$stmt = Conexion::conectar()->prepare(
			"SELECT h.id, h.modelo, h.accion, h.fecha, h.usuario_id, h.origen, h.observacion,
			 ...
			 LEFT JOIN categoria_modelojf ca ON ca.id = sa.id_categoria
			 LEFT JOIN subcategoria_modelojf sn ON sn.id = h.id_subcategoria_nueva
			 LEFT JOIN categoria_modelojf cn ON cn.id = sn.id_categoria
	...
	static public function mdlListarCategoriasAdmin()
	{
		$stmt = Conexion::conectar()->prepare(
			"SELECT c.*,
				...
				(SELECT COUNT(*) FROM subcategoria_modelojf s
				 INNER JOIN modelo_subcategoriajf ms ON ms.id_subcategoria = s.id
				 INNER JOIN modelojf m ON TRIM(m.modelo) = ms.modelo
				   AND UPPER(TRIM(IFNULL(m.estado, ''))) = 'ACTIVO'
				 WHERE s.id_categoria = c.id) AS modelos_activos
			 FROM categoria_modelojf c
