    ...
    public function mostrarTablaMateriaPrimaPaginado()
    {
        ...
        if (count($materiaprima) > 0) {
            foreach ($materiaprima as $item) {
                ...
                $proveedorFull = trim(str_replace('"', '', $item["Proveedores"]));
                ...
                if ($item["estpro"] == "Inactivo") {
                    ...
                    $botones = "<div class='mp-acciones'>"
                        . "<button class='btn btn-xs btn-info btnVisualizarArticulos' title='Visualizar Articulos' data-toggle='modal' data-target='#modalVisualizarArticulos' articuloMP='" . $item["CodPro"] . "'><i class='fa fa-eye'></i></button>"
                        . "<button class='btn btn-xs btn-default btnOrdenesMp' title='Ver OC / OS' data-toggle='modal' data-target='#modalOrdenesMp' codpro='" . $item["CodPro"] . "'><i class='fa fa-file-text-o'></i></button>"
                        . "<button class='btn btn-xs btn-warning btnEditarMateriaPrima' idMateriaPrima='" . $item["CodPro"] . "' data-toggle='modal' data-target='#modalEditarMateriaPrima' title='Editar Materia Prima'><i class='fa fa-pencil'></i></button>"
                        . "<button class='btn btn-xs btn-success btnDuplicarMateriaPrima' idMateriaPrima='" . $item["CodPro"] . "' data-toggle='modal' data-target='#modalDuplicarMateriaPrima' title='Nuevo Color'><i class='fa fa-clone'></i></button>"
                        . "<button class='btn btn-xs btn-primary btnEditarCosto' title='Visualizar Costo' data-toggle='modal' data-target='#modalEditarCostos' materiaPrima='" . $item["CodPro"] . "'><i class='fa fa-money'></i></button>"
                        . "<button class='btn btn-xs btn-danger btnAnularMateriaPrima' title='Anular Materia Prima' idMateriaPrima='" . $item["CodPro"] . "'><i class='fa fa-times'></i></button>"
                        . "</div>";
                }
