...
function vascoGestionClienteJson($respuesta)
{
...
if ($accion === "listar-pendientes") {
    ...
    vascoGestionClienteJson(ControladorVascoSync::ctrListarGestionClientePendiente($filtros));
    exit;
}
...
if ($accion === "procesar") {
    ...
    if (!is_array($payload)) {
        vascoGestionClienteJson(array("ok" => false, "msg" => "Body JSON inválido"));
        exit;
    }
    ...
    vascoGestionClienteJson(ControladorVascoSync::ctrProcesarGestionCliente($items, $ackBy, $traceId));
    exit;
}

vascoGestionClienteJson(array("ok" => false, "msg" => "Acción no reconocida"));
